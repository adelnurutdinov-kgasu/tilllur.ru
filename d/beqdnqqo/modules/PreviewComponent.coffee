...
`window.receiveMessageFill = function (event) {
	window.previewMessageFramerObject.animateStateToFill()
}
window.addEventListener("animateFill", receiveMessageFill, false);
`
...
# 		set: (value) -> @options.showBar = value
	...
	setDesktopScaleMode: (forState = "fill") =>
		scaleX = (Canvas.width - 120) / @width
		scaleY = (Canvas.height - 80) / @height
		@states.fill.scale = Math.min(scaleX, scaleY)
		@stateSwitch(forState)
		
# 		button = new Layer
# 			size: 64, borderRadius: 32
