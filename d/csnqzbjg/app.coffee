...
backpackButtons = (visibleButtons = 0) ->
	...
	suggestView_Color.states.lines.height = sumY
...
updateSuggestLines = (dataArray = [], maxResults = 6) ->
	...
	if !isEmptyInputText() then visibleLines = 0
...
placeLines = (visibleLinesCounter = 0) ->
	...
	if suggestView_LineView.children != undefined and suggestView_LineView.children.length != 0
		suggestView_Exit.y = suggestView_LineView.y + suggestView_LineView.children[0].height * visibleLinesCounter
		suggestView_Color.height = suggestView_Exit.y
	
	if isEmptyInputText()
		suggestView_Color.height += 16
...
inputText_Null.text = "найдётся всё"
inputText_Null.x = 0
inputText_Null.y = 0
...
closeIcon.states =
	"hidden": { opacity: 0, x: Align.right(300) }
	"shown": { opacity: .5, x: Align.right }
closeIcon.stateSwitch("hidden")
