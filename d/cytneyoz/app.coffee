...
toast_shape.on Events.StateSwitchEnd, (fromState, toState) ->
	if toState is "shown" and currentPage == 3
		...
		if isLoginQueue() and tick.states.current.name is "shown"
			zen_card.animate("shown", { delay: 1, time: 0.4 })
...
for i in [1...30]
	zen_item = new Layer
		...
		y: Math.floor((i - 1) / 3) * (132+12)*retina + 198*retina
...
search_view = new Layer
	...
	width: 200*retina
	height: 68*retina
	x: 80*retina - 65*retina + 16*retina
