...
sceneLayers = [figmaView, content, verticals, contentYandex, contentVideo, header, notifications, mail, user, ya, titleYandex, titleVideo, titleMarket, bottomBar, statusBar]
...
searchMarket.states =
	"shown": { opacity: 1 }
	"hidden": { opacity: 0 }
searchMarket.stateSwitch("hidden")

page2.on Events.Tap, ->
	if searchMarket.states.current.name == "shown" then nextState = "hidden" else nextState = "shown"
	searchMarket.stateSwitch(nextState)
