...
# 	bookmarks = [bookmarks, site_buttons_main, site_buttons_secondary]
...
createScroll = () ->
	...
	for item, i in [0, 1, 2, 3, 4]
		if !isCurrentGlobalModeSite and i == localSitePlace + 1
			...
			site_in_tab = new Layer width: 494, height: 820, image: "images/site in tab.png", x: i*(247+8)*2 + 64*2, parent: scrollView.content
...
# 	scrollView.animate { properties: { opacity: 1, x: 0 }, curve: tightSpring }
	scrollView.animate { properties: { x: 0 }, curve: "linear", time: 0.2 }
...
# 		serp_content.states.switch("shown", time: 0)
	
# 	Utils.delay 0.3, ->
# 		scrollView.opacity = 0
# 		tabs_buttons.opacity = 0
# 		
# 		localLogo = logo.copy()
# 		localLogo.parent = new_tab
# 		localLogo.y = 39*2
