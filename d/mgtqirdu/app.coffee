...
for item, i in [[left01, right01], [left02, right02], [right03]]
	contentView = new Layer
		width: 360
		height: leftScroll.height
		parent: leftScroll.content
		x: i * 360
		backgroundColor: "null"
	
	for widget, j in item
		if item != null
			widget.parent = contentView
			widget.x = (4 + 174) * j + 4
...
footer_temp = new Layer
	width: 360
	height: 18
	image: "images/footer%20temp.png"
	parent: card01
	y: Align.bottom()
...
handleAction = (event, layer) ->
	if leftScroll.content.children[0].children[0] == left01
		
		
		for item in leftScroll.content.children
			item.destroy()
		
		for item, i in [[left02, left01], [right01, right02], [right03]]
			contentView = new Layer
				width: 360
				height: leftScroll.height
				parent: leftScroll.content
...
panel.header("Alerts", "left")
panel.button("Tease", handleTeaser, "left", "tease")
panel.button("Add New", handleAction, "left", "tease")
