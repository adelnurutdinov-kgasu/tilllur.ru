...
screen = new Layer
	width: 360
	height: 640
	image: "https://bro-bg-store.s3.yandex.net/b231b628-60de-4b31-8f7b-9de0d264e078.jpg"
	clip: true
	borderRadius: 8
...
feedOverlayTop.states =
	"newTab": { opacity: 0, y: 640 }
	"feed": { opacity: 1, y: 640 - 48 }
...
logo.states =
	"newTab": { opacity: 1, y: 124 }
	"feed": { opacity: 0, y: 124 + 84 }
	"sites": { opacity: 0, y: 124 }
...
tabs.states =
	"newTab": { opacity: 1, y: 480 }
	"sites": { opacity: 0, y: 480 }
	"feed": { opacity: 0, y: 480 + 84 }
...
cardsView.states =
	"newTab": { opacity: 1, y: 264 + 64 }
	"sites": { opacity: 1, y: -64-16 + 64 }
	"feed": { opacity: 0, y: 264 + 84 + 64 }

cardsView.stateSwitch("newTab")
...
toFeedGap = [640, 1138] #[0, 1]
toFeedGapFast = [640, 640 + 120] #[0, 0.3]
toFeedGapMedium = [640, 640 + 200] #[0, 0.6]
toFeedGapLate = [1138 - 200, 1138 - 40] #[0, 1]
	...
	if isSwipeToFeed(v)
		...
		l[d] = Utils.modulate(v, g, [l.states[s1][d], l.states[s2][d]],
			true) for [l, d, g] in [[pageBot, "scale", toFeedGap],
									[feedOverlayBottom, "opacity", toFeedGapLate],
