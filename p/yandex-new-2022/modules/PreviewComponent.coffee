...
class LogoLayer extends SVGLayer
	constructor: (@options={}) ->
...
# 		set: (value) -> @options.showBar = value
	...
	getLocationData: () =>
		...
		for item in queryArray
			...
			if keyPart == "scale"
				if valuePart == "fill" then @stateSwitchToFill()
				else if valuePart == "normal" then @stateSwitchToNormal()
	...
	setDesktopScaleMode: (forState = "normal") =>
		...
		for item in location.search[1..].split('&')
			...
			if keyPart == "scale"
				if valuePart == "fill" then initState = "fill"
				else initState = "normal"
		...
		for item in location.search[1..].split('&')
			...
			if keyPart == "button"
				if valuePart == "off" then shouldShowButton = false
				else if valuePart == "false" then shouldShowButton = false
		...
		for item in location.search[1..].split('&')
			...
			if keyPart == "logo"
				if valuePart == "off" then shouldShowLogo = false
				else if valuePart == "false" then shouldShowLogo = false
		
		if shouldShowLogo then @createLogoButton(initState)
		if shouldShowButton then @createScaleButton(initState)
		@stateSwitch(initState)
	
	
	
	createLogoButton: (forState) =>
		if Utils.isFramerStudio() then return
		
		openHomeHandler = () ->
			window.location = "https://tilllur.ru"
