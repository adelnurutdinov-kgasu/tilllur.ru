...
class SearchEngine {
    constructor() {
        ...
        this.aliceAppPromoThreshold = thresholdStored !== null && !isNaN(Number(thresholdStored)) ? Number(thresholdStored) : 20;
    ...
    handleInput(e) {
        ...
        this.currentQuery.innerHTML = `<span class="suggestion-icon"><img src="svg/search.svg" alt="Поиск" width="20" height="20" /></span><span class="suggestion-text">${this.escapeHtml(query)}</span><span class="suggestion-hint">— Найти в Яндексе</span>`;
        this.aliceQuery.innerHTML = `<span class="suggestion-icon"><img src="svg/alice.svg" alt="Алиса" width="20" height="20" /></span><span class="suggestion-text">${this.escapeHtml(query)}</span><span class="suggestion-hint alice-hint">— Спросить у Алисы</span>`;
    ...
    updateUIState() {
        ...
        if (query === "") {
            ...
            // Обновляем кнопку сразу, так как саджест уже будет активен
            this.updateFooterButtonByActive(); 
        }
    }
    ...
    renderSuggestions() {
        ...
        this.suggestions.forEach((suggestion, idx) => {
            ...
            div.innerHTML = `<span class="suggestion-icon"><img src="svg/search.svg" alt="Поиск" width="20" height="20" /></span><span class="suggestion-text">${this.escapeHtml(suggestion.text)}</span>`;
    ...
    getAliceURL(query) {
        return `yandexbrowser://search?text=${encodeURIComponent(query)}&suggest_reqid=1&is_suggest=true&intent=alice_from_search`;
    }
