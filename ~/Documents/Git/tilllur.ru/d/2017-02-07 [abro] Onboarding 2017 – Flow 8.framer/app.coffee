...
videoPlayingHelper = false

backgroundsArray = []
# backround number to let it be [0..N]
totalPages = 4
...
# 	ignoreEvents: true
# 
# colored_welcome.states =
# 	"color-1": { backgroundColor: new Color("rgba(255, 163, 153, 0.1)") }
# 	"color-2": { backgroundColor: new Color("rgba(255, 153, 194, 0.1)") }
# 
# colored_welcome.on Events.StateSwitchEnd, (fromState, toState) ->
# 	if toState is "color-1" then colored_welcome.animate("color-2", {time: 2, colorModel: "hsl"})
# 	else if toState is "color-2" then colored_welcome.animate("color-1", {time: 2, colorModel: "hsl"})
# colored_welcome.stateSwitch("color-1")
...
selectedQueue = []
if isLoginQueue() then selectedQueue = [yandex_bg_video, iphoneContainer, optionalContainer, zen_done_screen, browser_screen]
else selectedQueue = [yandex_bg_video, iphoneContainer, optionalContainer, zen_screen, browser_screen]
...
if isLoginQueue() then zen_screen.x = 360*retina
else zen_done_screen.x = 360*retina
...
toast_shape.on Events.StateSwitchEnd, (fromState, toState) ->
	if toState is "shown" and currentPage == 3
		...
		if isLoginQueue() and tick.states.current.name is "shown"
			zen_card.animate("home", { delay: 1, time: 0.4 })
		else if !isLoginQueue()
			zen_card.animate("home", { delay: 1, time: 0.4 })
...
# 		opacity: 0
# 
# zen_logo.stateSwitch("hidden")


zenTime = 0.8
for item in [zen_card]
	item.animationOptions =
		curve: "spring(200, 20, 10)"

# zen_card.on Events.StateSwitchStart, (fromState, toState) ->
# 	if toState is "shown"
# 		zen_logo.animate("shown", time: zenTime)
# 	else if toState is "home"
# 		zen_logo.animate("home", time: zenTime)

zen_card.on Events.StateSwitchEnd, (fromState, toState) ->
	if toState is "shown"
		zen_card.animate("home", delay: zenTime/2)
...
zen_shadow = new Layer
	width: 360*retina
	height: 80*retina
	x: 0*retina
	image: "images/zen shadow.png"
...
zen_shadow.stateSwitch("zen_hidden")

zen_button = new Layer
	width: 360*retina
	height: 56*retina
	x: 0*retina
	image: "images/zen button.png"
...
zen_button.stateSwitch("zen_hidden")



for item in [zen_button, zen_shadow]
	item.parent = zen_screen
	item.animationOptions = 
		curve: "spring(200, 30, 10)"
...
# 
buttonTexts = [
	"Больше источников – лучше рекомендации",
	"Рекомендуем выбрать еще 1 источник",
	"Рекомендуем выбрать еще 2 источника",
	"Рекомендуем выбрать еще 3 источника",
	"Рекомендуем выбрать еще 4 источника"
]
...
for i in [1...30]
	zen_item = new Layer
		...
		y: Math.floor((i - 1) / 3) * (132+12)*retina + 214*retina
...
teaseZenChoosing = () ->
	Utils.delay 1, ->
		if isLoginQueue() then tick.stateSwitch("shown")
		else
			zenChangeItemState()
			Utils.delay 2, ->
				zen_button.animate("zen_shown")
				zen_shadow.animate("zen_shown")

# Zen Done Logic: iPhone Logic

zen_phone = new Layer
	parent: zen_done_screen
	width: 254*retina
	height: 410*retina
	x: 53*retina
	y: 204*retina
...
zen_darker_area = new Layer
	width: 228*retina
	height: 298*retina
	x: 0*retina
	y: -1*retina

zen_darker_area.states =
	"zen_home":
...
zen_darker_area.stateSwitch("zen_home")
...
search_view.stateSwitch("zen_home")

zen_plate = new Layer
	width: 202*retina
	height: 472*retina
	x: 13*retina
	image: "images/zen plate.png"
...
zen_plate.stateSwitch("zen_home")




zen_background.parent = zen_animation_view
zenPhoneArray = [zen_darker_area, search_view, zen_plate]
for item in zenPhoneArray
	item.parent = zen_animation_view

zen_plate.on Events.StateSwitchEnd, (fromState, toState, event, layer) ->
	if toState is "zen_base"
		Utils.delay 2, ->
			if tick.states.current.name == "shown"
				zen_plate.animate("zen_scrolled", time: 2)
	else if toState is "zen_scrolled"
		Utils.delay 1, ->
			if tick.states.current.name == "shown"
				zen_plate.animate("zen_base", time: 2)



# Zen Done Logic: Tick View

# tick_on_shadow = new Layer
# 	parent: zen_done_screen
# 	width: 360*retina
# 	height: 120*retina
# 	x: 0*retina
# 	y: 416*retina
# 	image: "images/tick on shadow.png"
# 
# tick_on_shadow.states =
# 	"shown":
# 		opacity: 0
# 	"init":
# 		opacity: 0
...
tick.on Events.StateSwitchEnd, (fromState, toState) ->
	if toState is "shown"
# 		configure_suggestions.stateSwitch("base")
		for item in zenPhoneArray
			try
				item.animate("zen_base")
	else if toState is "init"
# 		configure_suggestions.stateSwitch("continue")
		for item in zenPhoneArray
			try
				item.animate("zen_home")

tick.on Events.Click, ->
	if tick.states.current.name == "shown"
		tick.stateSwitch("init")
	else tick.stateSwitch("shown")

# tick.stateSwitch("shown")



zen_next_button = new Layer
	parent: zen_done_screen
	width: 360*retina
	height: 56*retina
	y: 536*retina
...
close_icon.propagateEvents = false
...
chooseBackgroundSlider = new ScrollComponent
	parent: iphoneContainer
	width: 360*retina
	height: 100*retina
	x: 0*retina
...
for i in [0..totalPages]
	item = new Layer
		...
		height: 100*retina
		x: 100*retina * i
		parent: chooseBackgroundSlider.content
	...
	if i is 0 then check.stateSwitch("selected")
	
	backgroundsArray.push(item)
	
	item.on Events.Click, (event, layer) ->
		backgroundItem.image = layer.image
		for item in backgroundsArray
			item.children[0].stateSwitch("base")
		layer.children[0].stateSwitch("selected")
...
import_bookmarks_icon = new Layer
	width: 120*retina
	height: 120*retina
	y: 248*retina
	image: "images/import bookmarks icon.png"

import_bookmarks_icon.states =
	"start":
		x: 8*retina
	"done":
		x: 120*retina

import_bookmarks_icon.stateSwitch("start")

import_history_icon = new Layer
	width: 120*retina
	height: 120*retina
	y: 248*retina
	image: "images/import history icon.png"

import_history_icon.states =
	"start":
		x: 232*retina
	"done":
		x: 120*retina

import_history_icon.stateSwitch("start")

import_browser_icon = new Layer
	image: "images/import browser icon.png"

import_browser_icon.states =
	"start":
		width: 128*retina
		height: 128*retina
		x: 116*retina
...
import_browser_icon.stateSwitch("start")

bookmarks_tick_block = new Layer
	width: 360*retina
	height: 80*retina
	x: 0*retina
	y: 456*retina
	image: "images/bookmarks tick block.png"

bookmarks_tick = new Layer
	width: 18*retina
	height: 18*retina
	x: 45*retina
	y: 467*retina
	image: "images/bookmarks tick.png"

bookmarks_tick.states =
	"start":
		opacity: 0
	"done":
		opacity: 1

bookmarks_tick.stateSwitch("start")



import_browser_icon.on Events.StateSwitchStart, (fromState, toState) ->
	if toState is "done"
		for item in [import_history_icon, import_bookmarks_icon]
			item.animate("done")
...
bookmarks_tick.stateSwitch("done")

for item in [import_history_icon, import_bookmarks_icon, import_browser_icon, bookmarks_tick_block, bookmarks_tick]
	item.parent = importContainer


bookmarks_tick.on Events.StateSwitchEnd, (fromState, toState) ->
	if toState is "done"
		import_browser_icon.animate("done")
	else if toState is "start"
		import_browser_icon.animate("start")

bookmarks_tick.on Events.Click, ->
	if bookmarks_tick.states.current.name is "done"
		bookmarks_tick.stateSwitch("start")
	else if bookmarks_tick.states.current.name is "start"
		bookmarks_tick.stateSwitch("done")
...
# 		background_fix_2.animate("shown", time: 0.2)
		navigationController.snapToNextPage()
		browser_screen.image = backgroundItem.image
		Utils.delay 1, ->
			if videoPlayingHelper
...
zen_next_button.on(Events.Click, nextButtonHandler)
...
progress_button.on(Events.Click, openFinishTutorialModal)

close_icon.on Events.Click, ->
	finish_tutorial.ignoreEvents = false
	modal_cancel_button.ignoreEvents = false
	modal_finish_button.ignoreEvents = false
	finish_tutorial.stateSwitch("shown", { time: 0.4 })
...
for item in [screen, navigationController, zen_done_screen, cardContainer, importContainer, android_back, navbar, status_bar, finish_tutorial]
